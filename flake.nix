                                                                                ...
                                                                                in
                                                                                    ''
                                                                                       ...
                                                                                       then
                                                                                            ...
                                                                                            # nix-collect-garbage
                                                                                            JSON="$( jq --compact-output --null-input --arg RELEASE "$RELEASE" 2--arg RELEASE_APPLICATION "$( ${ pkgs.which }/bin/which release-application )" --arg HASH "$HASH" --arg INDEX "$INDEX" --arg STANDARD_OUTPUT "$STANDARD_OUTPUT" --arg TEMPORARY "$TEMPORARY" --arg TYPE release '{ "release" : $RELEASE , "release-application" : $RELEASE_APPLICATION , "hash" : $HASH , "index" : $INDEX , "standard-output" : $STANDARD_OUTPUT , "temporary" : $TEMPORARY , "type" : $TYPE }' )" || failure 201f8f4f
                                                                                            if [[ -n "$RELEASE" ]]
                                                                                            then
